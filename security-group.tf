...
resource "aws_security_group_rule" "egress" {
  ...
  source_security_group_id = lookup(each.value, "source_security_group_id", "") == "" ? null : lookup(each.value, "source_security_group_id")
  cidr_blocks              = lookup(each.value, "cidr_blocks", "") == "" ? null : lookup(each.value, "cidr_blocks")
  ipv6_cidr_blocks         = try(lookup(each.value, "ipv6_cidr_blocks", "") == "" ? null : lookup(each.value, "ipv6_cidr_blocks"))
